...
def _fetch_clean_yahoo(name: str, symbol: str, start: str, end_date: str) -> pd.DataFrame | None:
    """
    从 Yahoo Finance 获取并清洗海外指数/ETF 数据。
    注意：end_date 是包含的（即会返回 end_date 当天的数据）
    """
    # === 关键修改：把 end_date 加 1 天，因为 yfinance 的 end 是不包含的 ===
    ...
    try:
        ...
        print(f"⚠️ {name} ({symbol}): Yahoo 下载失败 - {e}")
        return None
    ...
    required_cols = ["open", "close", "high", "low", "volume"]
    if not all(col in df.columns for col in required_cols):
        print(f"⚠️ {name} ({symbol}): 缺少必要列，实际列: {list(df.columns)}")
        return None
